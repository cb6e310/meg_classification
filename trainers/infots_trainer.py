...
from models.losses import InfoNCE, CrossEntropyLoss
...
class InfoTSTrainer:
    def __init__(
        ...
        self.unsup_pred = torch.nn.Linear(
            cfg.MODEL.ARGS.N_FEATURES, cfg.SOLVER.BATCH_SIZE
        ).cuda()

        self.train_labels = TensorDataset(
            torch.arange(len(train_loader.dataset)).to(torch.long).cuda()
        )
        self.cls_optimizer = torch.optim.AdamW(
            self.unsup_pred.parameters(), lr=self.cfg.SOLVER.META_LR
        )
    ...
    def meta_fit(self):
        logger.info("meta fitting...")
        pre_flag = self.model.training
        self.model.eval()
        for batch in self.train_loader:
            ...
            x = x.cuda()
            y = torch.arange(x.shape[0], dtype=torch.int64).cuda()
            self.meta_optimizer.zero_grad()

            x = torch.squeeze(x, -1)
            aug1, aug2 = self.aug(x)
            aug1 = aug1.unsqueeze(-1)
            aug2 = aug2.unsqueeze(-1)
            outv, outx, _, _ = self.model(aug1, aug2)
            MI_vx_loss = self.InfoNCE(outv, outx)  # minimize v1v2_MI maximize v1v2_loss

            pred_yv = torch.squeeze(self.unsup_pred(outv), 1)
            pred_yx = torch.squeeze(self.unsup_pred(outx), 1)

            MI_vy_loss = self.CE(pred_yv, y)
            MI_xy_loss = self.CE(pred_yx, y)

            vx_vy_loss = (MI_vy_loss + MI_xy_loss)
        ...
        logger.info("meta fitting completes")
